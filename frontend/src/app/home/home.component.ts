...
import {
  PopupEventRequestComponent
} from "../components/popups/create-popups/popup-event-request/popup-event-request.component";
import {EventRequest} from "../models/eventRequest";
import {EventRequestListComponent} from "../components/list-items/event-request-list/event-request-list.component";
...
export class HomeComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    //Event
    this.apiService.getEventRequest().subscribe((eventRequests) => {
      this.eventRequests = eventRequests;
    });

    this.apiService.refreshEventRequest$.subscribe(() => {
      this.apiService.getEvent().subscribe((eventRequests) => {
        this.eventRequests = eventRequests;
      });
    });
  ...
  refreshHomeEventRequests() {
    this.apiService.refreshEventRequest$.next();
  }
  ...
  openEventRequestPopup() {
    const dialogRef = this.dialog.open(PopupEventRequestComponent, {
      width: '40%',
      height: 'auto',
      enterAnimationDuration: 200,
      exitAnimationDuration: 200,
      data: {
        title: 'Create new eventRequest!',
        teams: this.teams,
      },
    });
    ...
    dialogRef.afterClosed().subscribe((result) => {
      if (result) {
        this.apiService.refreshEventRequest$.next(); // Trigger refreshEvents if result === true, so an event was created.
      }
    });
  }
}
